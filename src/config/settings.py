...
class Settings(BaseSettings):
    ...
    rapira_api_url: str = Field(
        default="https://api.rapira.exchange",
        description="Rapira API base URL"
    )
    rapira_api_key: str = Field(..., description="Rapira API key")
    ...
    # Cache Configuration
    rate_cache_ttl: int = Field(
        default=300,
        description="Rate cache TTL in seconds"
    )
    
    # Markup Configuration
    default_markup_rate: float = Field(
        default=2.5,
        description="Default markup rate in percentage"
    )
    ...
    default_manager_id: int = Field(..., description="Default manager user ID")
